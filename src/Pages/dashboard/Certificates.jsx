...
const CertCard = ({ cert, onDelete }) => {
  ...
  return (
    <div className="relative group">
      <div className="absolute -inset-0.5 bg-gradient-to-r from-[#d4d4d8] to-[#fafafa] rounded-2xl blur opacity-10 group-hover:opacity-30 transition duration-500" />
      <div className="relative bg-white/5 border border-white/12 rounded-2xl overflow-hidden">
        ...
        {imgLoaded && (
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-400/20 border border-gray-400/30 text-gray-300 text-xs w-full justify-center hover:bg-gray-400/30 transition-colors"
            >
      ...
      <Card>
        <div className="p-5 sm:p-6 space-y-4">
          <h2 className="text-sm font-semibold text-white flex items-center gap-2">
            <Plus className="w-4 h-4 text-gray-300" /> Upload Certificate
          </h2>
          ...
          <label
            ...
            className={`flex flex-col items-center justify-center w-full min-h-[160px] rounded-xl border-2 border-dashed cursor-pointer transition-all duration-300 ${
              dragOver ? 'border-gray-300/60 bg-gray-400/10' : 'border-white/12 bg-white/4 hover:border-gray-400/35 hover:bg-white/7'
            }`}
          >
            {preview ? (
              ...
              <div className="text-center space-y-2 p-6">
                <div className="w-11 h-11 rounded-full bg-gray-400/10 border border-gray-400/20 flex items-center justify-center mx-auto">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button onClick={uploadImage} disabled={uploading} className="relative group/u">
                  <div className="absolute -inset-0.5 bg-gradient-to-r from-[#9ca3af] to-[#a1a1aa] rounded-xl opacity-60 blur group-hover/u:opacity-100 transition duration-300" />
                  <div className="relative flex items-center gap-2 px-4 py-1.5 bg-[#030014] rounded-xl border border-white/10">
                    {uploading ? <div className="w-3.5 h-3.5 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <Upload className="w-3.5 h-3.5 text-gray-300" />}
